...
\begin{document}
	...
	Este método segundo \citeonline{incremental-learning}, é ideal para treinamento incremental e apenas alguns classificadores suportam este método, sendo assim, podemos extender a lógica do early stopping para alguns indicadores que não suportam tal método nativamente, como o Naive Bayes.
	
	Como o método incremental, tende a adicionar ruído na base de dados, foi utilizado o método de IIR, que segundo \citeonline{iir-concepts} é um filtro recursivo em que a saída do filtro é calculada usando as entradas atuais e anteriores e as saídas anteriores, fazendo assim com que o ruído seja minimizado, esta técnica foi utilizada para reduzir o ruído no tratamento de erro tanto da base de validação quanto da base de teste.
	...
	\begin{anexosenv}
		...
		\begin{figure}[htb]
			\caption{\label{fig_smooth_example}Comparação do Erro com e sem IIR}
			\begin{center}
		...
		\begin{figure}[htb]
			\caption{\label{with_overfitting}Gráfico de treinamento sem detectar Overfitting}
			\begin{center}
		...
		\begin{figure}[htb]
			\caption{\label{without_overfitting}Gráfico de treinamento com a detecção de  Overfitting}
			\begin{center}
